// 옥스치과(hospital_id=3, oaksdental) 전 페이지 "전문용어 자동 링크 + 호버 툴팁 + 하단 토스트바"
// - _footer 페이지(hospital_id=3)의 custom_css / custom_js 에만 가산적 패치
// - 센티넬 마커로 멱등(재실행 시 해당 블록만 교체). 기존 내용 보존.
// - hospital_id=3 외 절대 영향 없음.
//
// 사용: node patch-oaks-glossary.mjs ./wonjudental.db
...
;(function(){
  ...
  var tip, toast;
  ready(function(){
    fetch('/api/board?type=board&groupSlug=encyclopedia')
...
async function main() {
  ...
  console.log('[옥스치과 용어 글로서리 패치 — hospital_id=' + HOSPITAL_ID + ']');
  ...
  if (!page) {
    console.error('[ABORT] hospital_id=3 의 _footer 페이지가 없습니다. 중단.');
    process.exitCode = 1; db.close(); return;
  }
...
const invokedDirectly = process.argv[1] && process.argv[1].replace(/\\/g, '/').endsWith('patch-oaks-glossary.mjs');
if (invokedDirectly) {
