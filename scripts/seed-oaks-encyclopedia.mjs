// 옥스치과의원(hospital_id=3, slug=oaksdental) 치과 백과사전 시드
// - 가산적 / 멱등(이미 있으면 skip). hospital_id=3 외 절대 영향 없음.
// - board_groups.slug='encyclopedia' (전역 UNIQUE — 다른 병원 미사용 확인됨)
// - 콘텐츠는 일반 정보 제공용 "초안". 의료 정확성은 병원(의료진) 검수 전제.
//
// 사용: node seed-oaks-encyclopedia.mjs ./wonjudental.db

import sqlite3 from 'sqlite3';

const HOSPITAL_ID = 3;
const GROUP_SLUG = 'encyclopedia';
...
// ── 콘텐츠(초안) ─────────────────────────────────────────────────────────────
// 일반 정보 제공용. 의료 정확성·표현은 옥스치과 의료진 검수 후 확정.
const NOTE = `<h2>참고 안내</h2>
<p>본 문서는 일반적인 이해를 돕기 위한 정보이며, 개인의 구강 상태에 따라 진단·치료 방법은 달라질 수 있습니다. 정확한 진단과 치료 계획은 옥스치과의원 의료진 상담을 통해 확인하시기 바랍니다.[*본 백과사전 콘텐츠는 옥스치과의원이 검수·관리합니다.]</p>`;
...
async function main() {
  console.log('[옥스치과 백과사전 시드 시작 — hospital_id=' + HOSPITAL_ID + ']');
  ...
  try {
    ...
    async function ensurePage(slug, title, content) {
      ...
      const r = await run(
        `INSERT INTO pages (hospital_id,slug,title,content,custom_css,custom_js,meta_title,meta_description,is_published,sort_order,page_type)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
        [HOSPITAL_ID, slug, title, content, '', '', title + ' | 옥스치과의원',
         '옥스치과의원 치과 백과사전 — 치아교정·임플란트·심미치료 등 치과 용어를 쉽게 설명합니다.', 1, 90, 'custom']
      );
    ...
    console.log('[완료] 옥스치과 백과사전 시드 종료');
  } catch (e) {
